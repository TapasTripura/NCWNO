...
for i, path in enumerate(data_path):
    print('Loading:',path)
    if i == 5: data.append( (MatReader(path).read_field('u')[:,::sub_ns,::sub_ns,:]).permute(3,0,1,2) )
    else: data.append( (MatReader(path).read_field('sol')[:,:,::sub,::sub]) )
...
model = WNO2d(width=width, level=level, input_dim=T0+2, hidden_dim=4, 
              space_len=[1,1], size=[S,S], label_lifting=2**4).to(device)
...
"""
torch.save(model, '/data/model/NCWNO_MultiPhysics_2D_L4_B25_S20_L4_W96_T10_Epoch200_v2')
...
with torch.no_grad():
    for i, case_loader in enumerate(test_loader):
        ...
        print('Case-{}, Case_test_step-{:0.4f}'.format( i, case_test_step/ntest/(T/step)) )
